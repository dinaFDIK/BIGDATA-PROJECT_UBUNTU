...
# INITIALIZE SPARK
# =========================
spark = SparkSession.builder.appName("EnterpriseBatchPipelineDemo").master("local[*]").getOrCreate()
...
df = spark.read.schema(schema).option("header", True).csv("data/raw/ecommerce_raw.csv")
...
df_clean = df_clean.dropna(subset=["transaction_id", "customer_id", "price", "quantity"])
df_clean = df_clean.filter((col("price") > 0) & (col("quantity") > 0))

# Transform date string to date type
...
df_curated = df_transformed.groupBy("category").agg(_sum("total_amount").alias("total_revenue"))

df_top_products = df_transformed.groupBy("product").agg(_sum("quantity").alias("total_quantity")).orderBy(desc("total_quantity")).limit(5)

df_avg_transaction = df_transformed.groupBy("customer_id").agg(avg("total_amount").alias("avg_transaction_value"))
...
df_transformed.write.mode("overwrite").partitionBy("category").parquet("data/clean/partitioned_by_category/")
